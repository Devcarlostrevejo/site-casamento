...
export const weddingInputSchema = z.object({
  ...
  ceremonyName: z.string().trim().max(150),
  ceremonyAddress: z.string().trim().max(300),
  ceremonyInstructions: z.string().trim().max(1_000),
  ceremonyMapsUrl: z.union([z.literal(''), z.url().max(1_000)]).nullable(),
